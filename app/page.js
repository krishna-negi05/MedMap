      ...
      <div className="relative z-10 max-w-6xl w-full px-6 py-12 md:py-20 flex flex-col items-center text-center">
        ...
        <div className="w-full mt-24 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-12 duration-700 delay-200">
          ...
          <FeatureCard 
            href="/tools"
            title="Med Toolkit"
            desc="Flashcards, Drug Interactions, Lab Interpreter, and Viva Voice."
            icon={Layers}
            color="blue"
          />
          <FeatureCard 
...
function FeatureCard({ href, title, desc, icon: Icon, color }) {
  ...
  // Color variants for the decorative blob
  const blobs = {
    purple: "bg-purple-50",
    teal: "bg-teal-50",
    blue: "bg-blue-50",
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 right-0 w-32 h-32 ${blobs[color]} rounded-bl-[4rem] -mr-8 -mt-8 transition-transform duration-500 group-hover:scale-150`}></div>
      
      {/* Icon */}
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-all duration-300 relative z-10 ${colors[color]} shadow-sm`}>
        <Icon size={28} />
      </div>
